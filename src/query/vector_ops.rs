...
mod tests {
    ...
    #[test]
    fn test_l2_squared_zero_vector() {
        let a = vec![0.0f32; 10];
        let b = vec![0.0f32; 10];
    ...
    #[test]
    fn test_cosine_similarity_identical_normalized() {
    ...
    fn test_cosine_similarity_orthogonal() {
        ...
        assert!(sim.abs() < 1e-6, "orthogonal vectors should have similarity near 0");
    }
    ...
    #[test]
    fn test_cosine_distance_equals_one_minus_similarity() {
        let a = vec![1.0f32, 0.0, 0.0];
        let b = vec![0.0f32, 1.0, 0.0];
        let sim = cosine_similarity(&a, &b);
        let dist = cosine_distance(&a, &b);
        assert!((dist - (1.0 - sim)).abs() < 1e-6, "cosine_distance must be 1 - similarity");
    }

    #[test]
    fn test_inner_product_correct() {
        let a = vec![1.0f32, 2.0, 3.0];
        let b = vec![4.0f32, 5.0, 6.0];
    ...
    #[test]
    fn test_distance_metric_from_str_all_variants() {
        ...
        assert_eq!(DistanceMetric::from_str("L2_SQUARED"), Some(DistanceMetric::L2Squared));
        assert_eq!(DistanceMetric::from_str("L1"), Some(DistanceMetric::L1));
        assert_eq!(DistanceMetric::from_str("MANHATTAN"), Some(DistanceMetric::L1));
        assert_eq!(DistanceMetric::from_str("LINF"), Some(DistanceMetric::LInf));
        assert_eq!(DistanceMetric::from_str("CHEBYSHEV"), Some(DistanceMetric::LInf));
        assert_eq!(DistanceMetric::from_str("DOT"), Some(DistanceMetric::InnerProduct));
        assert_eq!(DistanceMetric::from_str("INNER_PRODUCT"), Some(DistanceMetric::InnerProduct));
        assert_eq!(DistanceMetric::from_str("COSINE"), Some(DistanceMetric::CosineSimilarity));
        assert_eq!(DistanceMetric::from_str("COSINE_DISTANCE"), Some(DistanceMetric::CosineDistance));
        assert_eq!(DistanceMetric::from_str("UNKNOWN_METRIC"), None);
        assert_eq!(DistanceMetric::from_str(""), None);
    }

    #[test]
    fn test_distance_computer_preserves_query() {
        let computer = DistanceComputer::new(DistanceMetric::L2, vec![1.0f32, 2.0, 3.0]);
        assert_eq!(computer.query, vec![1.0, 2.0, 3.0]);
        assert_eq!(computer.metric, DistanceMetric::L2);
    }

    #[test]
    fn test_distance_computer_cosine_precomputes_norm() {
        let computer = DistanceComputer::new(DistanceMetric::CosineSimilarity, vec![1.0f32, 0.0, 0.0]);
        // ||[1,0,0]|| = 1, so query_norm_sq = 1.0, query_norm_recip = 1.0
        assert!((computer.query_norm_sq - 1.0).abs() < 1e-6);
        assert!((computer.query_norm_recip - 1.0).abs() < 1e-6);
    }

    #[test]
    fn test_distance_computer_non_cosine_has_zero_recip() {
        let computer = DistanceComputer::new(DistanceMetric::L2, vec![1.0f32, 2.0, 3.0]);
        assert_eq!(computer.query_norm_recip, 0.0);
    }

    #[test]
    fn test_bytes_to_query_vec_f32_valid() {
        let bytes: Vec<u8> = vec![0u8, 0, 128, 63, 0, 0, 0, 64]; // [1.0, 2.0] in LE
        let result = bytes_to_query_vec_f32(&bytes).unwrap();
    ...
    #[test]
    fn test_bytes_to_query_vec_f32_invalid_length() {
        let bytes = vec![0u8, 1, 2]; // not divisible by 4
        assert!(bytes_to_query_vec_f32(&bytes).is_none());
    }

    #[test]
    fn test_bytes_to_query_vec_f32_single_float() {
        // 1 float32 = 4 bytes: 0.0 in little-endian
